...
const WebRegistration = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // ✅ FIX: Upload ID image after temp registration is created
      // Note: For web registration, we store the image with temp_patient_id
      // The image will be linked to actual patient_id when they complete at kiosk
      if (capturedIDImage && tempPatientId) {
        ...
        const idImageUrl = await uploadIDImage(
          capturedIDImage,
          tempPatientId,  // ✅ Use temp_patient_id for now
          selectedIDType
        );
